...
export class ExtendedAccountData {
  ...
  totalUSD: number | undefined = undefined; //lastBalance+inThePool * $UNC price
  ...
  async refreshLastBalance() {
    ...
    try {
      if (this.accountInfo.type == "lock.c") {
        ...
        this.accountInfo.lockedOther = yton(locked) + 35 // 35 $UNC are locked in lock-accounts to backup storage
      }
    } catch (ex) {
      console.error(ex)
    }
